...
**When the project has a testing rule** (e.g. `.claude/rules/testing.md` or `**/*.spec.*` / `**/*.test.*` globs): follow that rule first; it may extend or override the guidelines below. The rule is the project’s source of truth for Vitest/Jest conventions, CLI, and structure.
